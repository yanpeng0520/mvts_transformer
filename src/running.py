...
class SupervisedRunner_Reconstruction(BaseRunner):
    """SupervisedRunner_Reconstruction: Runner for supervised learning with reconstruction loss"""
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            #targets = targets.to(self.device)
            # now the targets are the input for reconstruction
            targets = X.to(self.device)
            padding_masks = padding_masks.to(self.device)  # 0s: ignore
